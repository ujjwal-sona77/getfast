    ...
    <>
      <nav className="fixed w-full z-40 bg-white/80 backdrop-blur-md shadow-lg">
        <div className="container mx-auto px-4 py-4">
          <a href="/shop" className="inline-flex items-center text-blue-600 hover:text-blue-800 transition-colors">
                  ...
                  <div key={item._id} className="flex flex-col md:flex-row items-center gap-4 p-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                    ...
                    <div className="flex-1 text-center md:text-left">
                      ...
                      <div className="mt-1">
                        {item.discount ? (
                          <>
                            <span className="text-green-600 font-bold">₹{(item.price - item.discount) * item.quantity}</span>
                            <span className="text-gray-400 line-through ml-2">₹{item.price * item.quantity}</span>
                          </>
                        ) : (
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex items-center border rounded-lg overflow-hidden bg-white">
                        <form action={`/cart/decrease/${item._id}`} method="post" className="inline">
                          <button type="submit" className="px-4 py-2 hover:bg-gray-100 transition-colors">-</button>
                        </form>
                        <span className="px-4 py-2 border-x">{item.quantity}</span>
                        <form action={`/cart/increase/${item._id}`} method="post" className="inline">
                          <button type="submit" className="px-4 py-2 hover:bg-gray-100 transition-colors">+</button>
                        </form>
                      </div>

                      <form action={`/cart/remove/${item._id}`} method="post" className="inline">
